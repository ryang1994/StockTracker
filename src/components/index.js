...
// Extracted so it can be called both by the scheduled sync job (see SALE DETECTION section)
// Formerly ran on every /api/items page load - moved to the scheduled job so it runs
// reliably on a real interval instead of only when someone happens to open the app.
async function runArchiveSweep() {
  ...
  return toArchiveResult.rows.length;
}

// GET all items (including their images)
app.get('/api/items', async (req, res) => {
  try {
    const requestedType = ['personal', 'damaged'].includes(req.query.type) ? req.query.type : 'stock';
...
app.post('/api/items', async (req, res) => {
  try {
    ...
    const resolvedItemType = ['personal', 'damaged'].includes(item_type) ? item_type : 'stock';

    for (let i = 0; i < qty; i++) {
      let itemNumber;
      if (resolvedItemType === 'personal') {
        const numResult = await pool.query("SELECT nextval('personal_item_number_seq') AS n");
        itemNumber = `PS-${String(numResult.rows[0].n).padStart(6, '0')}`;
      } else {
        const numResult = await pool.query("SELECT nextval('item_number_seq') AS n");
        itemNumber = `CL-${String(numResult.rows[0].n).padStart(6, '0')}`;
      }
...
app.patch('/api/items/:id/status', async (req, res) => {
  try {
    ...
    const currentResult = await pool.query('SELECT status, item_type FROM items WHERE id = $1', [id]);
    if (currentResult.rows.length === 0) {
      return res.status(404).json({ error: 'Item not found' });
    }
    const current = currentResult.rows[0];

    // Keep item_type in sync when moving to/from DAMAGED via this shared dropdown -
    // otherwise an item flipped straight from Active/Listed/Draft would end up with
    // status=DAMAGED but item_type still 'stock', which the dedicated Damaged tab,
    // dashboard exclusions and CSV write-off section all key off item_type, not status.
    // Never touches Personal items, so a personal write-off never counts as a business cost.
    let itemType = current.item_type;
    if (status === 'DAMAGED' && current.item_type === 'stock') {
      itemType = 'damaged';
    } else if (current.status === 'DAMAGED' && status !== 'DAMAGED' && current.item_type === 'damaged') {
      itemType = 'stock';
    }

    const result = await pool.query(
      `UPDATE items 
       SET status = $1, item_type = $2, updated_at = NOW() 
       WHERE id = $3 
       RETURNING *`,
      [status, itemType, id]
    );
...
// Withdraws an item's live eBay offer, if it genuinely has one. Shared by the
// automatic Vinted-sale withdrawal and the manual "Delist from eBay" button,
// so both go through the exact same tested logic rather than two copies.
async function withdrawEbayListing(item) {
  if (!item.active_on_ebay || !item.ebay_offer_id) {
    return { attempted: false };
  }

  try {
    const token = await getValidEbayAccessToken();
    const withdrawRes = await fetch(`${EBAY_API_BASE}/sell/inventory/v1/offer/${item.ebay_offer_id}/withdraw`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json', 'Accept-Language': 'en-GB' }
    });
    ...
    console.error('eBay withdrawOffer failed:', JSON.stringify(withdrawData));
    return { attempted: true, success: false, details: withdrawData };
  } catch (withdrawErr) {
    console.error('eBay withdrawOffer error:', withdrawErr.message);
    return { attempted: true, success: false, details: { message: withdrawErr.message } };
  }
}
...
app.patch('/api/items/:id/sell', async (req, res) => {
  try {
    ...
    let ebayWithdrawal = null;

    // If this sold on Vinted while still genuinely live on eBay, automatically end
    // the stale eBay listing too - otherwise it stays up for sale even though the
    // physical stock is already gone. Never blocks recording the sale if this fails;
    // the frontend surfaces success/failure to the user either way.
    if (sold_platform === 'Vinted') {
      ebayWithdrawal = await withdrawEbayListing(item);
      if (ebayWithdrawal.success) {
        const updateResult = await pool.query(
          `UPDATE items SET active_on_ebay = FALSE, updated_at = NOW() WHERE id = $1 RETURNING *`,
          [id]
        );
    ...
    res.json({ ...item, generated_description: generateDescription(item), ebayWithdrawal });
  } catch (err) {
    console.error(err);
    ...
    const result = await pool.query(
      `UPDATE items 
       SET status = 'DRAFT', 
           ...
           vinted_delisted = FALSE,
           vinted_delisted_at = NULL,
           updated_at = NOW()
       WHERE id = $1 
       RETURNING *`,
      [id]
...
app.get('/api/bookkeeping/summary', async (req, res) => {
  try {
    ...
    // Includes DAMAGED write-offs (genuine cost of goods that never sold) but not Personal
    // items (never business stock in the first place) - this filter was previously missing
    // entirely, which meant Personal item costs were being silently counted here too.
    const stockCostResult = await pool.query(
      `SELECT COALESCE(SUM(purchase_cost), 0) AS total, COUNT(*) FILTER (WHERE purchase_receipt_key IS NOT NULL) AS receipts FROM items
       WHERE created_at >= $1 AND created_at <= $2 AND item_type IN ('stock', 'damaged')`,
      [start, end]
    );
...
app.get('/api/bookkeeping/export', async (req, res) => {
  try {
    ...
    csv += '\nDAMAGED / WRITE-OFF STOCK (cost still counted as cost of goods above)\n';
    csv += 'Item Number,Date Added,Brand,Category,Purchase Cost,Has Photo Evidence\n';
    const damagedResult = await pool.query(
      `SELECT items.item_number, items.created_at, items.brand, items.category, items.purchase_cost,
        EXISTS(SELECT 1 FROM images WHERE images.item_id = items.id AND images.is_deleted = FALSE) AS has_photo
       FROM items
       WHERE item_type = 'damaged' AND created_at >= $1 AND created_at <= $2
       ORDER BY created_at ASC`,
      [start, end]
    );
    for (const row of damagedResult.rows) {
      csv += [
        csvEscape(row.item_number),
...
// Bundles every receipt image referenced in the tax-year CSV (item purchases, mileage,
// general expenses) into one zip - so if HMRC or an accountant ever asks for evidence,
// there's one self-contained package to hand over rather than live links into the app
// (which would otherwise sit behind the site's Basic Auth and be useless to anyone
// outside it). Files that no longer exist on disk are skipped rather than failing
// the whole export, since a missing receipt shouldn't block getting the rest.
app.get('/api/bookkeeping/export/receipts-zip', async (req, res) => {
  try {
    const startYear = parseInt(req.query.taxYear, 10) || getCurrentTaxYearStartYear();
    const { start, end } = getTaxYearBounds(startYear);
    const label = `${startYear}-${startYear + 1}`;
    ...
    res.setHeader('Content-Disposition', `attachment; filename="stocktracker-receipts-${label}.zip"`);

    const archive = archiver('zip', { zlib: { level: 9 } });
    archive.on('error', (err) => { throw err; });
    archive.pipe(res);

    const addIfExists = (relativeKey, zipFilename) => {
      if (!relativeKey) return;
      const fullPath = path.join(__dirname, relativeKey);
      if (fs.existsSync(fullPath)) {
        archive.file(fullPath, { name: zipFilename });
      } else {
        console.error('Receipt file missing on disk, skipped from zip:', fullPath);
      }
    };

    const itemsResult = await pool.query(
      `SELECT item_number, purchase_receipt_key FROM items
       WHERE status IN ('SOLD', 'DISPATCHED', 'ARCHIVED') AND date_sold >= $1 AND date_sold <= $2 
       AND item_type = 'stock' AND purchase_receipt_key IS NOT NULL`,
      [start, end]
    );
    for (const row of itemsResult.rows) {
      const ext = path.extname(row.purchase_receipt_key) || '.jpg';
      addIfExists(row.purchase_receipt_key, `items/${row.item_number}-purchase-receipt${ext}`);
    }

    const mileageResult = await pool.query(
      `SELECT trip_date, purpose, receipt_key FROM mileage_log 
       WHERE trip_date >= $1 AND trip_date <= $2 AND receipt_key IS NOT NULL`,
      [start, end]
    );
    for (const row of mileageResult.rows) {
      const dateStr = new Date(row.trip_date).toISOString().slice(0, 10);
      const ext = path.extname(row.receipt_key) || '.jpg';
      const safePurpose = (row.purpose || 'trip').replace(/[^a-z0-9]/gi, '-').slice(0, 40);
      addIfExists(row.receipt_key, `mileage/${dateStr}-${safePurpose}${ext}`);
    }

    const expensesResult = await pool.query(
      `SELECT expense_date, category, receipt_key FROM expenses 
       WHERE expense_date >= $1 AND expense_date <= $2 AND receipt_key IS NOT NULL`,
      [start, end]
    );
    for (const row of expensesResult.rows) {
      const dateStr = new Date(row.expense_date).toISOString().slice(0, 10);
      const ext = path.extname(row.receipt_key) || '.jpg';
      const safeCategory = (row.category || 'expense').replace(/[^a-z0-9]/gi, '-').slice(0, 40);
      addIfExists(row.receipt_key, `expenses/${dateStr}-${safeCategory}${ext}`);
    }

    await archive.finalize();
  } catch (err) {
    console.error('Receipt zip export error:', err);
    if (!res.headersSent) {
      res.status(500).json({ error: 'Failed to export receipts', details: err.message });
    }
  }
});
...
// ===================== SALE DETECTION & SCHEDULED SYNC =====================
// Checks eBay's Fulfillment API for new orders since the last check, matches each
// order line's SKU back to items.item_number, and auto-fills the sale fields on a
// match - the same fields "Mark Sold" fills in manually. Only matches items that
// are currently ACTIVE (genuinely live on eBay) so a stray/duplicate order can
// never accidentally re-sell something that's already sold or still a draft.
//
// The actual SKU-match-and-update logic lives in processOrderLineItem() below,
// separated out from the eBay API call itself for clarity and testability.
async function processOrderLineItem(order, lineItem) {
  const sku = lineItem.sku;
  if (!sku) return { matched: false, reason: 'Line item has no SKU' };

  const itemResult = await pool.query(
    `SELECT id FROM items WHERE item_number = $1 AND status = 'ACTIVE' AND item_type = 'stock'`,
    [sku]
  );

  if (itemResult.rows.length === 0) {
    return { matched: false, reason: `No ACTIVE stock item found for SKU ${sku}` };
  }

  const soldValue = (lineItem.total && lineItem.total.value) || (lineItem.lineItemCost && lineItem.lineItemCost.value);

  await pool.query(
    `UPDATE items 
     SET status = 'SOLD', sold_price = $1, selling_fees = 0, sold_platform = 'eBay', 
         date_sold = $2, ebay_order_id = $3, updated_at = NOW()
     WHERE id = $4`,
    [soldValue || null, order.creationDate || new Date().toISOString(), order.orderId, itemResult.rows[0].id]
  );

  return { matched: true, itemId: itemResult.rows[0].id, soldValue };
}

async function checkEbayOrders() {
  const summary = { ordersFound: 0, itemsMatched: 0, errors: [] };

  const settingsResult = await pool.query(
    `SELECT value FROM app_settings WHERE key = 'last_order_check_at'`
  );
  // First run ever: only look back 24 hours rather than eBay's full 90-day
  // default, so we don't sweep in old Sandbox test orders from earlier sessions.
  const since = settingsResult.rows.length > 0
  ...
  const response = await fetch(
    `${EBAY_API_BASE}/sell/fulfillment/v1/order?filter=creationdate:[${since}..]&limit=50`,
    {
      headers: {
  ...
  const data = await response.json();
  if (!response.ok) {
    throw new Error('eBay getOrders failed: ' + JSON.stringify(data));
  }
  ...
  for (const order of orders) {
    for (const lineItem of (order.lineItems || [])) {
      try {
        const result = await processOrderLineItem(order, lineItem);
        if (result.matched) summary.itemsMatched++;
      } catch (itemErr) {
        // One bad line item shouldn't stop the rest of the batch from processing
        console.error('Failed to process order line item:', order.orderId, lineItem.sku, itemErr.message);
        summary.errors.push(`${order.orderId} (${lineItem.sku}): ${itemErr.message}`);
      }
    }
  }

  await pool.query(
    `INSERT INTO app_settings (key, value) VALUES ('last_order_check_at', $1)
     ON CONFLICT (key) DO UPDATE SET value = $1`,
    [checkStartedAt]
  );
...
// Called by the systemd timer every couple of minutes (see deploy notes). Self-throttles
// against sale_check_interval_minutes in app_settings, so the actual interval is genuinely
// adjustable from Settings without ever touching the systemd timer file itself.
app.post('/api/sync/run', async (req, res) => {
  try {
    const settingsResult = await pool.query(
      `SELECT key, value FROM app_settings WHERE key IN ('last_sync_at', 'sale_check_interval_minutes')`
    );
    const settings = {};
    settingsResult.rows.forEach(row => { settings[row.key] = row.value; });

    const intervalMinutes = parseInt(settings.sale_check_interval_minutes, 10) || 15;
    const lastSync = settings.last_sync_at ? new Date(settings.last_sync_at) : null;
    const dueAt = lastSync ? lastSync.getTime() + intervalMinutes * 60 * 1000 : 0;

    if (Date.now() < dueAt) {
      return res.json({ skipped: true, nextRunAt: new Date(dueAt).toISOString() });
    }

    let orderCheck = { ordersFound: 0, itemsMatched: 0, errors: [] };
    try {
      orderCheck = await checkEbayOrders();
    } catch (err) {
      console.error('Sale detection failed:', err.message);
      orderCheck.errors.push(err.message);
    }

    let archivedCount = 0;
    try {
      archivedCount = await runArchiveSweep();
    } catch (err) {
    ...
    await pool.query(
      `INSERT INTO app_settings (key, value) VALUES ('last_sync_at', $1)
       ON CONFLICT (key) DO UPDATE SET value = $1`,
      [ranAt]
    );

    res.json({ skipped: false, ranAt, orderCheck, archivedCount });
  } catch (err) {
    console.error('Sync run failed:', err);
    res.status(500).json({ error: 'Sync run failed', details: err.message });
  }
});

// For the Settings page - shows when the sync last actually ran and when it's next due
app.get('/api/sync/status', async (req, res) => {
  try {
    ...
    const intervalMinutes = parseInt(settings.sale_check_interval_minutes, 10) || 15;
    const lastSyncAt = settings.last_sync_at || null;
    const nextRunAt = lastSyncAt
      ? new Date(new Date(lastSyncAt).getTime() + intervalMinutes * 60 * 1000).toISOString()
      : null;

    res.json({ lastSyncAt, intervalMinutes, nextRunAt });
  } catch (err) {
    console.error(err);
...
// Manual "Delist from eBay" - ends the live listing on demand, regardless of whether
// anything has actually sold. Frontend is responsible for its own confirm-are-you-sure
// prompt before calling this, since it's a real, immediate, irreversible action on eBay.
app.post('/api/items/:id/withdraw-ebay', async (req, res) => {
  try {
    const { id } = req.params;
    const itemResult = await pool.query('SELECT * FROM items WHERE id = $1', [id]);
    if (itemResult.rows.length === 0) {
    ...
    if (!item.active_on_ebay || !item.ebay_offer_id) {
      return res.status(400).json({ error: 'This item is not currently marked active on eBay' });
    }

    const ebayWithdrawal = await withdrawEbayListing(item);

    if (!ebayWithdrawal.success) {
      return res.status(502).json({ error: 'Failed to withdraw eBay listing', ebayWithdrawal });
    }
    ...
    res.json({ ...updateResult.rows[0], generated_description: generateDescription(updateResult.rows[0]), ebayWithdrawal });
  } catch (err) {
    console.error('Manual eBay withdraw error:', err);
    res.status(500).json({ error: 'Failed to withdraw eBay listing', details: err.message });
  }
});
...
app.patch('/api/items/:id/marketplace-status', async (req, res) => {
  try {
    const { id } = req.params;
    const { active_on_ebay, active_on_vinted, vinted_delisted } = req.body;
    ...
    if (vinted_delisted !== undefined) {
      fields.push(`vinted_delisted = $${idx++}`);
      values.push(vinted_delisted);
      // Stamp the moment it's confirmed; clear it if ever toggled back off
      fields.push(`vinted_delisted_at = ${vinted_delisted ? 'NOW()' : 'NULL'}`);
    }
...
app.get('/api/ebay/inventory-location/status', async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) {
      return res.json({ configured: false, merchantLocationKey: null });
    }
    const merchantLocationKey = result.rows[0].value;

    // Genuinely ask eBay, rather than trusting that our saved key still resolves to
    // something real - a Sandbox reset (or any account-side change) can silently
    // invalidate this without ever touching our own database.
    const token = await getValidEbayAccessToken();
    ...
    if (response.ok) {
      return res.json({ configured: true, merchantLocationKey });
    }

    console.error('Stored inventory location no longer valid on eBay:', response.status);
    res.json({ configured: false, merchantLocationKey, reason: 'Saved location no longer exists on eBay - may need to be recreated' });
  } catch (err) {
    console.error(err);
...
app.get('/api/ebay/business-policies/status', async (req, res) => {
  try {
    ...
    const optedIn = programs.includes('SELLING_POLICY_MANAGEMENT');

    // Also genuinely verify each stored policy ID still resolves on eBay's side -
    // a Sandbox reset (or any account-side change) can silently invalidate a saved
    // ID without our own database ever finding out, which is exactly what caused
    // today's confusing publish failures instead of an obvious warning here.
    let policiesValid = false;
    const invalidPolicies = [];

    if (optedIn) {
      const settingsResult = await pool.query(
        `SELECT key, value FROM app_settings WHERE key IN ('ebay_fulfillment_policy_id', 'ebay_return_policy_id', 'ebay_payment_policy_id')`
      );
      const settings = {};
      settingsResult.rows.forEach(row => { settings[row.key] = row.value; });

      const checks = [
        { key: 'ebay_fulfillment_policy_id', label: 'fulfillment', path: 'fulfillment_policy' },
        { key: 'ebay_return_policy_id', label: 'return', path: 'return_policy' },
        { key: 'ebay_payment_policy_id', label: 'payment', path: 'payment_policy' }
      ];

      const hasAllIds = checks.every(c => settings[c.key]);
      if (hasAllIds) {
        const results = await Promise.all(checks.map(c =>
          fetch(`${EBAY_API_BASE}/sell/account/v1/${c.path}/${settings[c.key]}`, {
            headers: { 'Authorization': `Bearer ${token}` }
          }).then(r => ({ label: c.label, ok: r.ok }))
        ));
        results.forEach(r => { if (!r.ok) invalidPolicies.push(r.label); });
        policiesValid = invalidPolicies.length === 0;
      }
    }

    res.json({ optedIn, programs, policiesValid, invalidPolicies });
  } catch (err) {
    console.error(err);
...
app.post('/api/ebay/business-policies/create', async (req, res) => {
  try {
    ...
    const existing = {};
    existingResult.rows.forEach(row => { existing[row.key] = row.value; });

    // Creates a policy, or - if eBay says one with that name already exists (errorId 20400) -
    // looks up the existing one by name instead of failing outright. This makes the whole
    // endpoint safe to re-run any time, including after a Sandbox reset that wipes some but
    // not all policies (as opposed to hard-failing the moment any single step hits a snag).
    async function createOrFindPolicy({ existingKey, createUrl, payload, listUrl, listField, idField, name }) {
      if (existing[existingKey]) {
        return { id: existing[existingKey], recovered: false, skipped: true };
      }
      ...
      if (createRes.ok) {
        return { id: createData[idField], recovered: false, skipped: false };
      }

      const isDuplicate = (createData.errors || []).some(e => e.errorId === 20400);
      if (!isDuplicate) {
        throw { step: existingKey, details: createData };
      }

      // Already exists on eBay's side under this name - look it up instead of failing
      const listRes = await fetch(`${listUrl}?marketplace_id=EBAY_GB`, { headers });
      const listData = await listRes.json();
      if (!listRes.ok) throw { step: existingKey, details: listData };

      const found = (listData[listField] || []).find(p => p.name === name);
      if (!found) throw { step: existingKey, details: { message: 'eBay reported a duplicate but it could not be found by name', createData, listData } };

      return { id: found[idField], recovered: true, skipped: false };
    }
    ...
    let fulfillment, returnPolicy, payment;
    try {
      fulfillment = await createOrFindPolicy({
        existingKey: 'ebay_fulfillment_policy_id',
        createUrl: `${EBAY_API_BASE}/sell/account/v1/fulfillment_policy`,
        payload: fulfillmentPayload,
        listUrl: `${EBAY_API_BASE}/sell/account/v1/fulfillment_policy`,
        listField: 'fulfillmentPolicies',
        idField: 'fulfillmentPolicyId',
        name: 'StockTracker Standard Shipping'
      });
      returnPolicy = await createOrFindPolicy({
        existingKey: 'ebay_return_policy_id',
        createUrl: `${EBAY_API_BASE}/sell/account/v1/return_policy`,
        payload: returnPayload,
        listUrl: `${EBAY_API_BASE}/sell/account/v1/return_policy`,
        listField: 'returnPolicies',
        idField: 'returnPolicyId',
        name: 'StockTracker Standard Returns'
      });
      payment = await createOrFindPolicy({
        existingKey: 'ebay_payment_policy_id',
        createUrl: `${EBAY_API_BASE}/sell/account/v1/payment_policy`,
        payload: paymentPayload,
        listUrl: `${EBAY_API_BASE}/sell/account/v1/payment_policy`,
        listField: 'paymentPolicies',
        idField: 'paymentPolicyId',
        name: 'StockTracker Standard Payment'
      });
    } catch (stepErr) {
      console.error('Business policy step failed:', stepErr.step, JSON.stringify(stepErr.details));
      return res.status(502).json({ error: `Failed at step: ${stepErr.step}`, details: stepErr.details, step: stepErr.step });
    }

    const updates = {
      ebay_fulfillment_policy_id: fulfillment.id,
      ebay_return_policy_id: returnPolicy.id,
    ...
    res.json({
      success: true,
      ...updates,
      status: { fulfillment: fulfillment.skipped ? 'already known' : fulfillment.recovered ? 'recovered by name' : 'newly created',
                returnPolicy: returnPolicy.skipped ? 'already known' : returnPolicy.recovered ? 'recovered by name' : 'newly created',
                payment: payment.skipped ? 'already known' : payment.recovered ? 'recovered by name' : 'newly created' }
    });
  } catch (err) {
...
// Looks up the real, current offer for a SKU directly from eBay and corrects our stored
// ebay_offer_id (and listing info) if it's drifted - e.g. after several publish attempts
// during troubleshooting left a stale ID behind from an earlier, since-abandoned offer.
app.post('/api/ebay/fix-offer-id', async (req, res) => {
  try {
    const { item_number } = req.body;
    if (!item_number) {
      return res.status(400).json({ error: 'Pass { "item_number": "CL-000007" } in the request body' });
    }

    const itemResult = await pool.query('SELECT id FROM items WHERE item_number = $1', [item_number]);
    if (itemResult.rows.length === 0) {
      return res.status(404).json({ error: 'Item not found' });
    }
    ...
    const token = await getValidEbayAccessToken();
    const response = await fetch(`${EBAY_API_BASE}/sell/inventory/v1/offer?sku=${encodeURIComponent(item_number)}`, {
      headers: { 'Authorization': `Bearer ${token}`, 'Accept-Language': 'en-GB' }
    });
    const data = await response.json();

    if (!response.ok) {
      return res.status(502).json({ error: 'Failed to look up offers for this SKU', details: data });
    }

    const offers = data.offers || [];
    if (offers.length === 0) {
      return res.status(404).json({ error: 'eBay has no offer at all on record for this SKU', data });
    }

    // Prefer a published offer if there's more than one on record
    const offer = offers.find(o => o.status === 'PUBLISHED') || offers[0];
    const listingId = offer.listing && offer.listing.listingId;
    const ebayUrl = listingId
      ? `https://www.${EBAY_ENV === 'production' ? '' : 'sandbox.'}ebay.com/itm/${listingId}`
      : null;

    const updateResult = await pool.query(
      `UPDATE items 
       SET ebay_offer_id = $1, 
           ebay_listing_id = COALESCE($2, ebay_listing_id), 
           ebay_url = COALESCE($3, ebay_url),
           updated_at = NOW()
       WHERE id = $4 
       RETURNING *`,
      [offer.offerId, listingId, ebayUrl, itemId]
    );

    res.json({ success: true, foundOffers: offers.length, usedOffer: offer, updated: updateResult.rows[0] });
  } catch (err) {
    console.error('Fix offer ID error:', err);
    res.status(500).json({ error: 'Failed to fix offer ID', details: err.message });
  }
});

// Recovers existing policy IDs from eBay directly, for when Sandbox resets the opt-in
// flag but leaves the underlying policies intact (as opposed to /create, which assumes
// no policies exist yet and fails with a duplicate-name error if they actually do).
app.post('/api/ebay/business-policies/resync', async (req, res) => {
  try {
    const token = await getValidEbayAccessToken();
    const headers = { 'Authorization': `Bearer ${token}`, 'Accept-Language': 'en-GB' };

    const [fulfillmentRes, returnRes, paymentRes] = await Promise.all([
      fetch(`${EBAY_API_BASE}/sell/account/v1/fulfillment_policy?marketplace_id=EBAY_GB`, { headers }),
      fetch(`${EBAY_API_BASE}/sell/account/v1/return_policy?marketplace_id=EBAY_GB`, { headers }),
      fetch(`${EBAY_API_BASE}/sell/account/v1/payment_policy?marketplace_id=EBAY_GB`, { headers })
    ]);

    const [fulfillmentData, returnData, paymentData] = await Promise.all([
      fulfillmentRes.json(), returnRes.json(), paymentRes.json()
    ]);

    if (!fulfillmentRes.ok || !returnRes.ok || !paymentRes.ok) {
      return res.status(502).json({
        error: 'Failed to list existing policies from eBay',
        details: { fulfillmentData, returnData, paymentData }
      });
    }

    const foundFulfillment = (fulfillmentData.fulfillmentPolicies || []).find(p => p.name === 'StockTracker Standard Shipping');
    const foundReturn = (returnData.returnPolicies || []).find(p => p.name === 'StockTracker Standard Returns');
    const foundPayment = (paymentData.paymentPolicies || []).find(p => p.name === 'StockTracker Standard Payment');

    const updates = {};
    if (foundFulfillment) updates.ebay_fulfillment_policy_id = foundFulfillment.fulfillmentPolicyId;
    if (foundReturn) updates.ebay_return_policy_id = foundReturn.returnPolicyId;
    if (foundPayment) updates.ebay_payment_policy_id = foundPayment.paymentPolicyId;
    ...
    res.json({
      ...
      recovered: updates,
      notFound: {
        fulfillment: !foundFulfillment,
        return: !foundReturn,
        payment: !foundPayment
      }
    });
  } catch (err) {
    console.error('Business policy resync error:', err);
    res.status(500).json({ error: 'Failed to resync business policies', details: err.message });
  }
});
